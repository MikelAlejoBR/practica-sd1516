...
int ping(char *ipaddr)
{
	...
	FILE *fp;
	int stat = 0;
	sprintf(command, "fping %s 2>&1", ipaddr);
	...
	if(fp == NULL) {
	fprintf(stderr, "Error al ejecutar fping\n");
	return -1;
	}
	...
	while(!feof(fp)) 
	{
	sz = fgets(buffer, 128, fp);

	// if(sz != NULL) result += buffer;
	}
...
void heartbeat(){
  ...
  while(1){	
	  ...
	  status = ping(ip);
	  if (!status) {
	    printf("Se ha recibido respuesta de %s\n", ip);
	    intentos=0;	
	  } else {
	    printf("Maquina no disponible\n");
            intentos++;	
	    if (intentos>=2)
		{
